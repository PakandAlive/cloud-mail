...
function md5Bytes(bytes) {
	...
	tail[bytes.length % 64] = 128;

	if (bytes.length % 64 > 55) {
		md5Cycle(state, bytesToWords(tail));
		tail.fill(0);
	}

	const bitLength = bytes.length * 8;
	const block = bytesToWords(tail);
	block[14] = bitLength & 0xffffffff;
	block[15] = Math.floor(bitLength / 0x100000000);
...
const md5Utils = {
	hex(input) {
		return md5Bytes(encoder.encode(input)).map(wordToHex).join('');
	}
};
